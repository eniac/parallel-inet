...
void TCPConnection::sendToIP(TCPSegment *tcpseg)
{
    ...
    if (!remoteAddr.isIPv6())
    {
        ...
        controlInfo->setExplicitCongestionNotification((state->ect && !state->sndAck && !state->rexmit) ? IP_ECN_ECT_1 : IP_ECN_NOT_ECT);
...
void TCPConnection::sendSynAck()
{
    ...
    if (state->ecnWillingness) {
        ...
        if (state->endPointIsWillingECN)
            tcpEV << "non-ECN-setup SYN-ACK packet sent\n";
    }
    if (state->ecnWillingness && state->endPointIsWillingECN) {
        state->ect = true;
        tcpEV << "both end-points are willing to use ECN... ECN is enabled\n";
    }
